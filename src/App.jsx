...
const allowLocalDemoMode = !supabaseEnabled; // local-only Topclass 버전에서는 Supabase 없이 동작합니다.
...
const INTEREST_UNIVERSITY_LIMIT = 10;
const GOAL_UNIVERSITY_LIMIT = 2;
const isGoalUniversity = (target, index = 0) => target?.isGoal === true || (target?.isGoal === undefined && index < GOAL_UNIVERSITY_LIMIT);
const goalUniversitiesForProfile = profile => (profile?.targets || [])
  .filter((target, index) => isGoalUniversity(target, index))
  .slice(0, GOAL_UNIVERSITY_LIMIT);
const normalizeTargetGoals = targets => {
  let goalCount = 0;
  return (targets || []).map((target, index) => {
    const requestedGoal = target?.isGoal === true || (target?.isGoal === undefined && index < GOAL_UNIVERSITY_LIMIT);
    const isGoal = requestedGoal && goalCount < GOAL_UNIVERSITY_LIMIT;
    if (isGoal) goalCount += 1;
    return target?.isGoal === isGoal ? target : { ...target, isGoal };
  });
};
const DEPT_PRESETS = {
...
const GRADE_YEAR_GROUPS = [
  { year:"고1", sems:["고1-1","고1-2"] },
  { year:"고2", sems:["고2-1","고2-2"] },
  { year:"고3", sems:["고3-1"] },
];
const defaultExpandedGradeYearsForLevel = gradeLevel => {
  const text = `${gradeLevel || ""}`;
  if (text.includes("3")) return ["고1", "고2", "고3"];
  if (text.includes("2")) return ["고1", "고2"];
  return ["고1"];
};
...
const buildSwot = (user, profile, score, diagnosis) => {
  ...
  const targets = goalUniversitiesForProfile(profile);
  const strengths = [];
  const weaknesses = [];
  const opportunities = [];
  const threats = [];
...
const buildSusiCards = (user, profile, catalog, avgValue) => {
  ...
  const seen = new Set();
  const preferred = goalUniversitiesForProfile(profile);
...
const buildStrategyReport = (user, profile, catalog = UNIVS, careerTests = CAREER_TESTS) => {
  ...
  const goalTargets = goalUniversitiesForProfile(profile);
  ...
  const targetScore = Math.min(100, goalTargets.length * 45);
  ...
  const targetStats = goalTargets.map(target => {
...
const STUDENT_NAV = [["홈","⌂","홈"],["목표대학","🏛","목표대학"],["성적표","📊","성적표"],["전략센터","✨","전략센터"],["생활기록부","📄","생활기록부"],["자소서","✍","자소서"],["로드맵","🧭","로드맵"],["과제점검","📝","과제 점검"]];
const CHECKLIST_ITEMS = [
  ...
  { id:"target-balance", month:"2026년 5월", title:"목표 대학 2곳 등록", body:"상향과 적정 중심으로 목표 대학 2곳을 정리합니다.", urgent:false },
  ...
  { id:"first-counseling", month:"2026년 6월", title:"전형 방향 점검", body:"목표 대학별 학과와 수시 전형 데이터를 다시 확인합니다.", urgent:false },
...
const gradeChartDomainForData = (rows, subjects = CORE) => {
  const values = (rows || [])
    .flatMap(row => subjects.map(subject => row?.[subject]))
    .filter(value => value !== undefined && value !== "" && !Number.isNaN(+value))
    .map(Number);
  if (!values.length) return [1, 9];
  const min = Math.min(...values);
  const max = Math.max(...values);
  let low = Math.max(1, Math.floor((min - 0.35) * 2) / 2);
  let high = Math.min(9, Math.ceil((max + 0.35) * 2) / 2);
  if (high - low < 1) {
    low = Math.max(1, low - 0.5);
    high = Math.min(9, high + 0.5);
  }
  return [low, high];
};

const gradeTicksForDomain = ([low, high]) => {
  const step = high - low <= 2.5 ? 0.5 : 1;
  const ticks = [];
  for (let value = Math.ceil(low / step) * step; value <= high + 0.001; value += step) {
...
body{font-family:'Noto Sans KR',sans-serif;background:var(--brand-bg);color:var(--brand-text);font-size:14px;line-height:1.55;overflow-x:hidden;}
...
.main{flex:1;min-width:0;overflow-y:auto;overflow-x:hidden;padding:32px 36px;}
...
.grade-overview-pill{display:inline-flex;align-items:center;gap:10px;border:1px solid rgba(14,165,233,0.20);border-radius:999px;background:#EAF7FF;color:#0B78D1;padding:9px 14px;margin:0 0 14px;font-size:12.5px;font-weight:800;}
.grade-overview-pill strong{font-size:15px;color:#0B1324;letter-spacing:0;}
.grade-entry-section{display:grid;gap:14px;}
.grade-section-head,.grade-average-head,.grade-chart-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:4px;}
.grade-year-stack{display:grid;gap:10px;}
.sn{display:flex;align-items:center;gap:7px;font-size:13.5px;}
.avrow{display:flex;gap:10px;margin-bottom:4px;flex-wrap:wrap;}
.avbox{flex:1;min-width:90px;background:#fff;border:1px solid #E5EAF1;border-radius:10px;padding:14px 8px;text-align:center;}
.grade-year-block{border:1px solid #E5EAF1;border-radius:16px;overflow:hidden;background:rgba(255,255,255,0.88);}
.grade-year-block.open{border-color:rgba(14,165,233,0.28);box-shadow:0 14px 32px rgba(14,165,233,0.08);}
.grade-year-toggle{width:100%;display:flex;align-items:center;gap:12px;padding:14px 16px;background:transparent;border:none;cursor:pointer;font-family:'Noto Sans KR',sans-serif;text-align:left;}
.grade-year-toggle:hover{background:#F8FAFC;}
.grade-year-label{font-size:15px;font-weight:900;color:#202632;min-width:38px;}
.grade-year-badge{font-size:12px;color:#0B78D1;font-weight:800;flex:1;line-height:1.45;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.grade-year-empty{font-size:12px;color:#9AA6B2;flex:1;font-weight:700;}
.grade-year-chevron{font-size:11.5px;color:#0EA5E9;margin-left:auto;font-weight:900;border:1px solid rgba(14,165,233,0.18);border-radius:999px;padding:5px 9px;background:#F5FBFF;white-space:nowrap;}
.grade-year-body{border-top:1px solid #EEF2F7;padding:14px;background:rgba(248,250,252,0.56);}
.grade-sem-grid{display:grid;gap:16px;}
.grade-sem-col{min-width:0;border:1px solid #EEF2F7;border-radius:14px;background:#fff;overflow:hidden;}
.grade-sem-header{display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:12px;font-weight:900;color:#202632;letter-spacing:0;margin:0;padding:11px 12px;border-bottom:1px solid #EEF2F7;background:#F8FAFC;}
.grade-sem-header em{font-style:normal;font-size:11.5px;font-weight:800;color:#0EA5E9;white-space:nowrap;}
.grade-compact-table{width:100%;}
.grade-compact-table th{padding:7px 8px;font-size:11px;color:#9AA6B2;font-weight:500;border-bottom:1px solid #EEF2F7;text-align:center;}
.grade-compact-table th:first-child{text-align:left;}
.grade-compact-table td{padding:4px 6px;border-bottom:1px solid #F7FAFD;text-align:center;}
.grade-compact-table td:first-child{text-align:left;}
.grade-compact-table tr:last-child td{border-bottom:none;}
.grade-compact-table input{width:58px;border:1px solid #E1E7EF;border-radius:8px;background:#fff;padding:7px 5px;text-align:center;font-size:12.5px;font-weight:700;color:#202632;font-family:'Noto Sans KR',sans-serif;outline:none;}
.grade-compact-table input:focus{border-color:#0EA5E9;box-shadow:0 0 0 3px rgba(14,165,233,0.08);}
.grade-compact-table input.grade-required{background:#F8FBFF;border-color:#BFDBFE;}
.grade-average-panel{margin-top:4px;border:1px solid #E5EAF1;border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,0.92),rgba(245,251,255,0.72));padding:16px;}
.grade-average-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;}
.grade-average-card{border:1px solid rgba(14,165,233,0.18);border-radius:14px;background:#fff;padding:12px;text-align:center;}
.grade-average-card span{display:block;font-size:11.5px;font-weight:900;color:#6B7684;margin-bottom:5px;}
.grade-average-card strong{display:block;font-size:21px;line-height:1.1;font-weight:900;color:#0B1324;letter-spacing:0;}
.grade-average-card small{display:block;margin-top:4px;font-size:10.5px;color:#0EA5E9;font-weight:900;}
.grade-average-card.empty{background:#F8FAFC;border-color:#EEF2F7;}
.grade-average-card.empty strong,.grade-average-card.empty small{color:#C9D3DF;}
.grade-chart-card{overflow:hidden;}
...
.profile-head{background:linear-gradient(135deg,rgba(255,255,255,0.94),rgba(245,251,255,0.78));border:1px solid rgba(229,234,241,0.92);border-radius:18px;padding:24px 26px;margin-bottom:16px;display:flex;flex-direction:column;gap:16px;}
.profile-head-top{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;width:100%;}
.profile-head-main{min-width:0;flex:1;}
.profile-title-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px;}
.profile-name{font-size:24px;font-weight:700;color:#202632;letter-spacing:-0.5px;margin-bottom:6px;}
.profile-title-row .profile-name{margin-bottom:0;}
.profile-edit-toggle{border:1px solid rgba(14,165,233,0.24);background:#EAF7FF;color:#0EA5E9;border-radius:999px;padding:7px 12px;font-size:12px;font-weight:900;cursor:pointer;font-family:'Noto Sans KR',sans-serif;}
.profile-edit-toggle:hover{border-color:#0EA5E9;background:#DFF3FF;}
.profile-head-edit{width:100%;border-top:1px solid #EEF2F7;padding-top:16px;}
.profile-head-edit .field{margin-bottom:0;}
.profile-meta{font-size:13px;color:#6B7684;display:flex;gap:8px;flex-wrap:wrap;margin:4px 0 14px;}
.profile-focus-grid{display:block;max-width:920px;}
.profile-goal-panel{border:1.5px solid rgba(14,165,233,0.18);background:linear-gradient(135deg,rgba(245,251,255,0.95),rgba(255,255,255,0.88));border-radius:18px;padding:16px 18px;box-shadow:0 14px 30px rgba(14,165,233,0.08);}
.profile-goal-strip{display:grid;grid-template-columns:repeat(2,minmax(230px,1fr));gap:12px;margin-top:10px;}
.profile-goal-chip{border:1px solid rgba(14,165,233,0.22);background:#fff;border-radius:15px;padding:18px 20px;display:grid;gap:5px;text-align:left;cursor:pointer;font-family:'Noto Sans KR',sans-serif;min-height:92px;box-shadow:0 10px 24px rgba(14,165,233,0.08);}
.profile-goal-chip:hover{border-color:#0EA5E9;background:#F5FBFF;}
.profile-goal-chip strong{font-size:18px;line-height:1.25;color:#0B1324;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.profile-goal-chip small{font-size:15px;line-height:1.35;color:#0B78D1;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.profile-goal-empty{display:inline-flex;align-items:center;border:1px dashed rgba(14,165,233,0.22);border-radius:12px;background:rgba(234,247,255,0.54);color:#0B78D1;font-size:12px;font-weight:800;padding:9px 11px;}
.profile-major-label{font-size:11px;font-weight:900;color:#0EA5E9;letter-spacing:0.02em;white-space:nowrap;}
.profile-micro-summary{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px;}
.profile-micro-summary span{display:inline-flex;align-items:center;border-radius:999px;background:rgba(248,250,252,0.9);border:1px solid #E5EAF1;color:#6B7684;font-size:11.5px;font-weight:800;padding:5px 9px;}
.profile-head-side{display:flex;align-items:flex-end;flex-direction:column;gap:8px;flex-shrink:0;}
.profile-head-side .score-pill{font-weight:900;}
.profile-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:12px;align-items:start;}
.profile-grid > *{min-width:0;}
.profile-section{background:#fff;border:1px solid #E5EAF1;border-radius:14px;padding:20px 22px;margin-bottom:12px;min-width:0;}
.profile-section > div[style*="overflow-x"]{max-width:100%;}
.profile-target-section{padding:26px 26px 22px;border-width:1.5px;box-shadow:0 24px 55px rgba(11,19,36,0.08);}
.profile-target-section .request-top{align-items:center;}
.profile-target-hero{border-color:rgba(14,165,233,0.26);background:linear-gradient(180deg,rgba(255,255,255,0.88),rgba(245,251,255,0.74));}
.profile-target-hero-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px;}
.profile-target-kicker{font-size:12px;font-weight:900;color:#0EA5E9;margin-bottom:7px;}
.profile-target-title{font-size:28px;line-height:1.24;font-weight:900;color:#0B1324;letter-spacing:0;margin-bottom:6px;}
.profile-target-copy{font-size:14px;line-height:1.65;color:#6B7684;}
.profile-target-search-panel{border:1px solid rgba(14,165,233,0.16);border-radius:18px;background:rgba(255,255,255,0.74);padding:16px;margin-bottom:18px;}
.profile-target-main-search input{height:44px;font-weight:700;}
.profile-target-filter-bar{margin-top:10px;}
.profile-target-results.expanded{grid-template-columns:repeat(auto-fill,minmax(220px,1fr));margin-top:10px;}
.profile-target-note{margin-top:12px;font-size:13px;font-weight:800;color:#0B78D1;}
.profile-target-warning{margin-top:12px;font-size:13px;font-weight:800;color:#B91C1C;}
.profile-target-workspace{display:grid;grid-template-columns:minmax(220px,280px) minmax(0,1fr);gap:16px;align-items:start;}
.profile-target-roster{display:grid;gap:10px;align-self:start;position:sticky;top:0;}
.profile-target-choice{width:100%;display:grid;gap:9px;border:1px solid #E5EAF1;border-radius:14px;background:rgba(255,255,255,0.88);padding:12px;text-align:left;font-family:'Noto Sans KR',sans-serif;transition:all 0.14s;}
.profile-target-choice:hover,.profile-target-choice.active{border-color:#0EA5E9;background:#F5FBFF;box-shadow:0 12px 26px rgba(14,165,233,0.10);}
.profile-target-open{width:100%;display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:10px;align-items:center;border:0;background:transparent;padding:0;text-align:left;cursor:pointer;font-family:'Noto Sans KR',sans-serif;color:inherit;}
.profile-target-choice strong{display:block;font-size:14px;line-height:1.3;color:#202632;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.profile-target-choice small{display:block;margin-top:3px;font-size:12px;line-height:1.4;color:#6B7684;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.profile-target-choice em{font-size:12px;font-style:normal;font-weight:900;white-space:nowrap;}
.profile-goal-toggle{justify-self:start;border:1px solid rgba(14,165,233,0.24);border-radius:999px;background:#fff;color:#0B78D1;font-size:11.5px;font-weight:900;padding:6px 10px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;}
.profile-goal-toggle:hover{border-color:#0EA5E9;background:#EAF7FF;}
.profile-goal-toggle.active{background:#EAF7FF;border-color:#0EA5E9;color:#0EA5E9;}
.profile-goal-toggle:disabled{cursor:not-allowed;background:#F1F5F9;border-color:#E5EAF1;color:#9AA6B2;}
.profile-goal-toggle.detail{justify-self:auto;white-space:nowrap;}
.profile-target-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap;}
.goal-badge{background:#EAF7FF;color:#0EA5E9;}
.profile-target-alert{border-radius:14px;padding:11px 12px;font-size:12.5px;line-height:1.55;font-weight:800;}
.profile-target-alert.soft{background:#FEF3C7;color:#92400E;}
.profile-target-alert.danger{background:#FEE2E2;color:#991B1B;}
.profile-target-detail-view{min-width:0;}
.profile-target-detail-head{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid #E5EAF1;border-radius:18px;background:rgba(255,255,255,0.86);padding:16px 18px;margin-bottom:12px;}
.profile-target-school-line{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px;}
.profile-target-school-line > span:first-child{font-size:19px;font-weight:900;color:#202632;letter-spacing:0;}
.profile-target-school-sub{font-size:13px;line-height:1.55;color:#6B7684;}
.profile-target-control-row{display:grid;grid-template-columns:minmax(220px,320px) minmax(0,1fr);gap:12px;align-items:stretch;margin-bottom:12px;}
.profile-target-control-row .field{margin-bottom:0;}
.profile-target-match{margin-bottom:0;min-height:68px;}
.profile-target-analysis-card{border:1px solid #E5EAF1;border-radius:18px;background:rgba(255,255,255,0.86);padding:18px;margin-bottom:12px;}
.profile-dept-stat-list{grid-template-columns:repeat(auto-fill,minmax(210px,1fr));max-height:360px;overflow:auto;padding-right:2px;}
.profile-target-track-card{background:rgba(255,255,255,0.86);}
.profile-target-empty{padding:26px 12px;}
.toggle-roadmap-btn{border:1px solid rgba(14,165,233,0.24);background:#ffffff;color:#0B78D1;padding:8px 13px;border-radius:999px;font-size:13px;font-weight:700;cursor:pointer;white-space:nowrap;}
.toggle-roadmap-btn.open{background:#EFF6FF;}
.roadmap-compact-summary{width:100%;display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid #E5EAF1;border-radius:16px;background:rgba(248,250,252,0.88);padding:13px 15px;text-align:left;cursor:pointer;font-family:'Noto Sans KR',sans-serif;color:#202632;}
.roadmap-compact-summary:hover{border-color:#0EA5E9;background:#F5FBFF;}
.roadmap-compact-progress{display:grid;place-items:center;width:54px;height:54px;border-radius:15px;background:#EAF2FF;color:#0EA5E9;font-size:17px;font-weight:900;}
.roadmap-compact-summary strong{display:block;font-size:14px;line-height:1.35;font-weight:900;color:#202632;}
.roadmap-compact-summary small{display:block;margin-top:3px;font-size:12.5px;line-height:1.45;color:#6B7684;}
.roadmap-compact-summary em{font-style:normal;font-size:12px;font-weight:900;color:#0B78D1;white-space:nowrap;}
...
.profile-target-result:hover,.profile-target-result.selected{border-color:#0EA5E9;background:#F5FBFF;}
...
.profile-target-add-btn:disabled{cursor:default;opacity:0.55;background:#CBD5E1!important;border-color:#CBD5E1!important;color:#fff!important;}
...
@media (max-width: 900px){
  ...
  .admin-student-row{grid-template-columns:1fr 1fr;}
	  ...
	  .profile-head-top{flex-direction:column;}
	  .profile-head-side{align-items:flex-start;}
	  .profile-focus-grid{max-width:none;width:100%;}
	  .profile-goal-strip{grid-template-columns:1fr;}
	  .profile-grid{grid-template-columns:1fr;}
  .profile-target-workspace{grid-template-columns:1fr;}
	  .profile-target-roster{position:static;}
	  .profile-target-control-row{grid-template-columns:1fr;}
	  .grade-section-head,.grade-average-head,.grade-chart-head{flex-direction:column;}
	  .grade-sem-grid{grid-template-columns:1fr !important;}
	  .grade-average-grid{grid-template-columns:repeat(2,minmax(0,1fr));}
...
@media (max-width: 760px){
  ...
  .card,.gsec,.profile-section,.profile-head{border-radius:12px;padding:16px;}
	  ...
	  .grade-average-grid{grid-template-columns:1fr 1fr;}
	  .grade-year-toggle{align-items:flex-start;}
	  .grade-year-badge{white-space:normal;}
  ...
  .profile-target-hero-head{flex-direction:column;}
  .profile-target-title{font-size:23px;}
  .profile-target-detail-head{grid-template-columns:1fr;align-items:flex-start;}
  .profile-target-school-line > span:first-child{font-size:17px;}
  .roadmap-compact-summary{grid-template-columns:auto minmax(0,1fr);}
  .roadmap-compact-summary em{grid-column:2;justify-self:start;}
...
export default function App() {
  ...
  const [currentUserId, setCurrentUserId] = useState(() => {
    const stored = allowLocalDemoMode ? readJson(SESSION_KEY, null) : null;
    return stored || null;
  });
  ...
  const [profileInfoEditOpen, setProfileInfoEditOpen] = useState(false);
  const [profileTargetDetailId, setProfileTargetDetailId] = useState(null);
  const [profileTargetPreview, setProfileTargetPreview] = useState(null);
  const [profileTargetTrackTab, setProfileTargetTrackTab] = useState("수시");
  ...
  const [newStudentForm, setNewStudentForm] = useState({ name:"", gradeLevel:"3학년", classId:"", highSchool:"", preferredMajor:"" });
  ...
  const [expandedGradeYears, setExpandedGradeYears] = useState(["고1"]);
  const [profileExpandedGradeYears, setProfileExpandedGradeYears] = useState(["고1"]);
  ...
  useEffect(() => {
    ...
    setFname(profile.fname || "");
	    ...
	    setSection("홈");
	    setExpandedGradeYears(defaultExpandedGradeYearsForLevel(currentUser.gradeLevel));
  ...
  useEffect(() => {
    ...
    setProfileInfoEditOpen(false);
    setProfileTargetDetailId(null);
    setProfileTargetPreview(null);
	    setProfileTargetTrackTab("수시");
	    setTargetNotice("");
	    setRoadmapOpen(false);
	    const profileUser = users.find(user => user.id === profileStudentId);
	    setProfileExpandedGradeYears(defaultExpandedGradeYearsForLevel(profileUser?.gradeLevel));
	  }, [profileStudentId]);
  ...
  const handleAuthSubmit = async e => {
    ...
    if (authMode === "login") {
      ...
      // Allow a simple local login for the Topclass counselor
      if ((email === "topclass" || email === "topclass@local") && password === "3535") {
        let demo = users.find(u => u.id === "counselor-demo");
        if (!demo) {
          demo = { id: "counselor-demo", name: "Topclass 상담사", email: "counselor@topclass.local", password: "3535", role: "counselor", isActive: true };
          const nextUsers = [...users, demo];
          setUsers(nextUsers);
  ...
  const allStudentRows = users
    ...
    .map((user, index) => {
      ...
      const avgValue = coreAvgForGrades(profile.grades || {});
      const primaryTarget = goalUniversitiesForProfile(profile)[0] || profile.targets?.[0];
  ...
  const profileJournals = profileStudent
    ...
    : [];
	  ...
	  const profileSemesterAverages = profileStudent ? SEMS.map(semester => ({
	    semester,
	    avg: semAvgForGrades(profileStudent.profile.grades || {}, semester),
	  })) : [];
	  const profileHasSemesterAverages = profileSemesterAverages.some(item => item.avg);
	  const profileGradeChartDomain = gradeChartDomainForData(profileChartData, CORE);
	  const profileGradeChartTicks = gradeTicksForDomain(profileGradeChartDomain);
  const allStudents = allStudentRows.map(row => row.user);
  const studentsWithTargets = allStudentRows.filter(row => goalUniversitiesForProfile(row.profile).length > 0);
  ...
  const profileGoalTargets = goalUniversitiesForProfile(profileStudent?.profile);
  const profileGoalIds = new Set(profileGoalTargets.map(item => item.id));
  const profileInterestLimitReached = profileTargets.length >= INTEREST_UNIVERSITY_LIMIT;
  const profileGoalLimitReached = profileGoalTargets.length >= GOAL_UNIVERSITY_LIMIT;
  const profileGoalComplete = profileGoalTargets.length >= GOAL_UNIVERSITY_LIMIT && profileGoalTargets.every(t => t.dept);
  ...
  const getMatch = univ => getMatchForAvg(univ, coreAvg());
	  ...
	  const semesterAverages = SEMS.map(semester => ({ semester, avg: semAvg(semester) }));
	  const hasSemesterAverages = semesterAverages.some(item => item.avg);
	  const gradeChartDomain = gradeChartDomainForData(chartData, CORE);
	  const gradeChartTicks = gradeTicksForDomain(gradeChartDomain);
	  const avg = coreAvg();
  ...
  const activeProfileSelectedTarget = profileTargets.find(item => item.id === profileTargetDetailId) || null;
  const activeProfileTarget = profileTargetPreview || activeProfileSelectedTarget || profileTargets[0] || null;
  const activeProfileTargetSelected = activeProfileTarget ? profileTargetIds.has(activeProfileTarget.id) : false;
  const activeProfileTargetGoal = activeProfileTarget ? profileGoalIds.has(activeProfileTarget.id) : false;
  const activeProfileTargetDepartments = activeProfileTarget
    ? [...new Set([
      ...departmentOptionsForUniversity(activeProfileTarget, apiMajors, exactDepartmentsForUniversity(activeProfileTarget)),
      activeProfileTarget.dept,
    ].filter(Boolean))]
    : [];
  const activeProfileTargetDeptDraft = activeProfileTarget ? selectedDeptByUniversity[activeProfileTarget.id] : "";
  const activeProfileTargetDept = activeProfileTarget
    ? (activeProfileTargetSelected && activeProfileTargetDepartments.includes(activeProfileTarget.dept)
      ? activeProfileTarget.dept
      : activeProfileTargetDepartments.includes(activeProfileTargetDeptDraft)
        ? activeProfileTargetDeptDraft
        : activeProfileTargetDepartments[0] || activeProfileTarget.dept || "")
    : "";
  const activeProfileStatsUniversity = activeProfileTarget
    ? { ...activeProfileTarget, depts: activeProfileTargetDepartments.length ? activeProfileTargetDepartments : activeProfileTarget.depts }
    : null;
  const activeProfileAdmissionStats = activeProfileStatsUniversity && activeProfileTargetDept ? getLastYearAdmissionStats(activeProfileStatsUniversity, activeProfileTargetDept) : null;
  const activeProfileDepartmentStats = activeProfileStatsUniversity
    ? (activeProfileTargetDepartments.length ? activeProfileTargetDepartments : activeProfileStatsUniversity.depts || []).slice(0, 80).map(dept => getLastYearAdmissionStats(activeProfileStatsUniversity, dept))
    : [];
  const activeProfileTargetMatch = activeProfileTarget ? getMatchForAvg(activeProfileTarget, profileStudent?.avgValue) : null;
  ...
  const createNewStudent = async () => {
    ...
    if (!name) {
      setBackendError("학생 이름을 입력해주세요.");
      return;
    }
    const classId = newStudentForm.classId || myCounselorClasses[0]?.id || "";
    const assignedClass = classById.get(classId);
    const studentId = createId("student");
    const nextUser = {
      id: studentId,
      name,
      email: `student-${studentId}@topclass.local`,
      role: "student",
      isActive: true,
      gradeLevel: newStudentForm.gradeLevel,
      className: assignedClass?.name || "",
      highSchool: newStudentForm.highSchool,
      preferredMajor: newStudentForm.preferredMajor,
    };
    const nextProfile = createProfile();
    const nextMembership = classId ? { studentId, classId, joinedAt: new Date().toISOString() } : null;
    const nextUsers = [...users, nextUser];
    const nextProfiles = { ...profiles, [studentId]: nextProfile };
    const nextMemberships = nextMembership ? [...classMemberships, nextMembership] : classMemberships;
    setUsers(nextUsers);
    setProfiles(nextProfiles);
    if (nextMembership) setClassMemberships(nextMemberships);
    if (supabaseEnabled) {
      try {
        await createBackendStudent(nextUser);
        if (nextMembership) await createBackendClassMembership(studentId, classId);
      } catch (error) {
      ...
      writeJson(USERS_KEY, nextUsers);
      writeJson(PROFILES_KEY, nextProfiles);
      if (nextMembership) writeJson(MEMBERSHIPS_KEY, nextMemberships);
    }
    setNewStudentForm({ name:"", gradeLevel:"3학년", classId: myCounselorClasses[0]?.id || "", highSchool:"", preferredMajor:"" });
  ...
  const openProfileTargetCandidate = university => {
    if (!university) return;
    const selectedTarget = profileTargets.find(item => item.id === university.id);
    setProfileTargetPreview(selectedTarget ? null : university);
    setProfileTargetDetailId(selectedTarget ? selectedTarget.id : null);
    setProfileTargetTrackTab("수시");
    setTargetNotice("");
  };
  ...
  const addProfileTarget = university => {
    if (!profileStudent) return;
    if (profileTargetIds.has(university.id)) {
      openProfileTargetCandidate(university);
      return;
    }
    const currentTargets = profileStudent.profile.targets || [];
    if (currentTargets.length >= INTEREST_UNIVERSITY_LIMIT) {
      setProfileTargetPreview(university);
      setProfileTargetDetailId(null);
      setTargetNotice(`관심대학은 최대 ${INTEREST_UNIVERSITY_LIMIT}개까지 추가할 수 있습니다. 기존 관심대학을 삭제하면 다시 추가할 수 있어요.`);
      return;
    }
    const departments = departmentOptionsForUniversity(university, apiMajors, exactDepartmentsForUniversity(university));
    const chosenDept = departments.includes(selectedDeptByUniversity[university.id]) ? selectedDeptByUniversity[university.id] : departments[0] || "";
    updateProfileStudentProfile(profile => ({
      ...profile,
      targets: [
        ...normalizeTargetGoals(profile.targets || []),
        { ...university, dept: chosenDept, isGoal: goalUniversitiesForProfile(profile).length < GOAL_UNIVERSITY_LIMIT },
      ].slice(0, INTEREST_UNIVERSITY_LIMIT),
    }));
    setProfileTargetDetailId(university.id);
    setProfileTargetPreview(null);
    setQuery("");
    setTargetNotice("");
  };
  ...
  const removeProfileTarget = id => {
    updateProfileStudentProfile(profile => ({
      ...profile,
      targets: normalizeTargetGoals((profile.targets || []).filter(item => item.id !== id)),
    }));
    if (profileTargetDetailId === id) setProfileTargetDetailId(null);
    if (profileTargetPreview?.id === id) setProfileTargetPreview(null);
  };

  const setProfileTargetGoal = (id, shouldGoal) => {
    if (shouldGoal && !profileGoalIds.has(id) && profileGoalTargets.length >= GOAL_UNIVERSITY_LIMIT) {
      setTargetNotice(`목표대학은 ${GOAL_UNIVERSITY_LIMIT}개까지만 표시할 수 있습니다. 다른 목표대학을 먼저 해제해 주세요.`);
      return;
    }
    updateProfileStudentProfile(profile => ({
      ...profile,
      targets: normalizeTargetGoals(profile.targets || []).map(item => item.id === id ? { ...item, isGoal: shouldGoal } : item),
    }));
    setTargetNotice("");
  };
  ...
  const setActiveProfileTargetDept = dept => {
    if (!activeProfileTarget) return;
    setSelectedDeptByUniversity(prev => ({ ...prev, [activeProfileTarget.id]: dept }));
    if (activeProfileTargetSelected) setProfileTargetDept(activeProfileTarget.id, dept);
  };
        ...
        <div className="auth-root">
          <div className="auth-shell">
            ...
            <form className="auth-card" onSubmit={handleAuthSubmit}>
              ...
              <div className="seg">
                <button type="button" className={`seg-btn active`} onClick={() => { setAuthMode("login"); setAuthError(""); setAuthNotice(""); }}>로그인</button>
              </div>

              <div className="field">
                <label htmlFor="auth-email">아이디</label>
                <input id="auth-email" className="auth-input" type="text" value={authForm.email} onChange={e => setAuthValue("email", e.target.value)} autoComplete="username" />
              </div>
              <div className="field">
              ...
              {allowLocalDemoMode && (
                <div className="ghost-row">
                  <button type="button" className="ghost-btn" onClick={() => {
                    const demo = users.find(u => u.id === "counselor-demo") || users.find(u => u.email === "counselor@test.com");
                    if (demo) loginWithUser(demo);
                  }}>체험하기</button>
                </div>
              )}
            </form>
            ...
            {counselorTab === "classes" ? (
              ...
              <>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, marginBottom:16 }}>
                  ...
                  <button className="small-secondary" type="button" style={{ color:"#B91C1C", borderColor:"#F87171" }} onClick={deleteCurrentStudent}>학생 삭제</button>
                </div>
                <div className="profile-head">
                    ...
                    <div className="profile-head-main">
	                      <div className="profile-title-row">
	                        <div className="profile-name">{profileStudent.user.name}</div>
		                        <button className="profile-edit-toggle" type="button" onClick={() => setProfileInfoEditOpen(open => !open)}>
		                          {profileInfoEditOpen ? "닫기" : "수정"}
		                        </button>
		                      </div>
	                      ...
	                      <div className="profile-focus-grid">
	                        <div className="profile-goal-panel">
	                          <div className="profile-major-label">목표 대학</div>
	                          <div className="profile-goal-strip" aria-label="목표대학 요약">
	                            {profileGoalTargets.map(target => (
	                              <button key={target.id} type="button" className="profile-goal-chip" onClick={() => { setProfileTargetPreview(null); setProfileTargetDetailId(target.id); }}>
	                                <strong>{target.name}</strong>
	                                <small>{target.dept || "학과 선택 필요"}</small>
	                              </button>
	                            ))}
	                            {!profileGoalTargets.length && <span className="profile-goal-empty">목표대학 2개를 표시하면 여기에 바로 보입니다</span>}
	                            {profileGoalTargets.length > 0 && profileGoalTargets.length < GOAL_UNIVERSITY_LIMIT && (
	                              <span className="profile-goal-empty">목표 {profileGoalTargets.length}/{GOAL_UNIVERSITY_LIMIT}</span>
	                            )}
	                          </div>
	                        </div>
	                      </div>
	                      <div className="profile-micro-summary" aria-label="학생 상태 요약">
	                        <span>목표 {profileGoalTargets.length}/{GOAL_UNIVERSITY_LIMIT}</span>
	                        ...
	                        <span>{profileStudent.match?.label || "상태 대기"}</span>
	                      </div>
		                    </div>
                  ...
                  {profileInfoEditOpen && (
                    <div className="profile-head-edit">
                ...
                <section className="profile-section profile-target-section profile-target-hero">
                  <div className="profile-target-hero-head">
                    <div>
	                      <div className="profile-target-kicker">최우선 관리</div>
	                      <div className="profile-target-title">목표 대학 및 전형 적합도</div>
	                      <div className="profile-target-copy">관심대학은 최대 10개까지 담고, 그중 2개를 목표대학으로 표시해 입결·학과·전형을 바로 비교합니다.</div>
	                    </div>
	                    <span className={`status-pill${profileGoalComplete ? " confirmed" : ""}`}>
	                      관심 {profileTargets.length}/{INTEREST_UNIVERSITY_LIMIT} · 목표 {profileGoalTargets.length}/{GOAL_UNIVERSITY_LIMIT}
	                    </span>
                  </div>

                  <div className="profile-target-search-panel">
                    <div className="sw compact profile-target-main-search">
                      <span className="ico">🔍</span>
                      <input type="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="대학명, 학과명, 지역 검색" />
                    </div>
                    <div className="univ-filter-bar profile-target-filter-bar">
                      <div className="univ-filter-field">
                        <label htmlFor="profile-target-region-filter">지역</label>
                        <select id="profile-target-region-filter" value={regionFilter} onChange={e => setRegionFilter(e.target.value)}>
                          <option value="all">전국</option>
                          {UNIVERSITY_REGIONS.map(region => <option key={region} value={region}>{region}</option>)}
                        </select>
                      </div>
                      <div className="univ-filter-field">
                        <label htmlFor="profile-target-department-filter">학과 계열</label>
                        <select id="profile-target-department-filter" value={departmentFilter} onChange={e => setDepartmentFilter(e.target.value)}>
                          {DEPARTMENT_FILTERS.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}
                        </select>
                      </div>
                      <div className="univ-filter-field">
                        <label htmlFor="profile-target-rank-filter">정렬</label>
                        <select id="profile-target-rank-filter" value={rankFilter} onChange={e => setRankFilter(e.target.value)}>
                    ...
                    {(query || activeUniversityFilters) && (
                      <div className="univ-result-meta">
                        ...
                        <span>{universityApiStatus === "careernet" ? `커리어넷 API 연결됨 · 대학 ${apiUniversities.length}개 · 학과 ${apiMajors.length}개` : universityApiStatus === "loading" ? "커리어넷 API 불러오는 중" : "내장 전국 데이터 사용 중"}</span>
                      </div>
                    )}
                    {(query || activeUniversityFilters) && (
                      <div className="profile-target-results expanded">
                        {filtered.slice(0, 12).map(u => {
                          const selected = profileTargetIds.has(u.id);
                          return (
	                            <button key={u.id} type="button" className={`profile-target-result${selected ? " selected" : ""}`} onClick={() => openProfileTargetCandidate(u)}>
	                              <span className="dot" style={{ background:u.color, width:9, height:9 }} />
	                              <span>{u.name}</span>
	                              <small>{selected ? "관심 등록됨" : profileInterestLimitReached ? "보기" : "관심 추가"}</small>
	                            </button>
                          );
                        })}
	                    ...
	                    {profileGoalLimitReached && (
	                      <div className="profile-target-note">목표대학 2개가 표시되어도 검색 결과와 관심대학 추가는 계속 사용할 수 있습니다.</div>
	                    )}
	                    {profileInterestLimitReached && (
	                      <div className="profile-target-note">관심대학 10개가 모두 찼습니다. 새 대학을 저장하려면 기존 관심대학을 삭제해 주세요.</div>
	                    )}
                    {targetNotice && <div className="profile-target-warning">{targetNotice}</div>}
                  </div>

                  <div className="profile-target-workspace">
	                    <aside className="profile-target-roster" aria-label="관심 대학">
	                      <div className="mini-title">관심 대학</div>
	                      {profileTargets.map(t => {
	                        const match = getMatchForAvg(t, profileStudent.avgValue);
	                        const isGoal = profileGoalIds.has(t.id);
	                        return (
	                          <div key={t.id} className={`profile-target-choice${activeProfileTargetSelected && activeProfileTarget?.id === t.id ? " active" : ""}`}>
	                            <button type="button" className="profile-target-open" onClick={() => { setProfileTargetPreview(null); setProfileTargetDetailId(t.id); }}>
	                              <span className="dot" style={{ background:t.color, width:10, height:10 }} />
	                              <span>
	                                <strong>{t.name}</strong>
	                                <small>{t.dept || "학과 선택 필요"}</small>
	                              </span>
	                              {match && <em style={{ color:match.color }}>{match.label}</em>}
	                            </button>
	                            <button
	                              type="button"
	                              className={`profile-goal-toggle${isGoal ? " active" : ""}`}
	                              onClick={() => setProfileTargetGoal(t.id, !isGoal)}
	                              disabled={!isGoal && profileGoalLimitReached}
	                            >
	                              {isGoal ? "목표대학" : profileGoalLimitReached ? "목표 2개" : "목표로 표시"}
	                            </button>
	                          </div>
	                        );
	                      })}
	                      {!profileTargets.length && <div className="empty profile-target-empty">관심 대학이 아직 없습니다</div>}
	                      {profileGoalTargets.length < GOAL_UNIVERSITY_LIMIT && (
	                        <div className="profile-target-alert soft">관심대학 중 목표대학 {GOAL_UNIVERSITY_LIMIT}개를 표시해 주세요.</div>
	                      )}
	                      {profileGoalTargets.some(t => !t.dept) && (
	                        <div className="profile-target-alert danger">학과가 비어 있는 목표 대학이 있습니다.</div>
	                      )}
                    </aside>

                    <div className="profile-target-detail-view">
                      {activeProfileTarget ? (
                        <>
                          <div className="profile-target-detail-head">
                            <div className="uavatar" style={{ background:activeProfileTarget.color }}>{activeProfileTarget.short.slice(0,3)}</div>
                            <div>
                              <div className="profile-target-school-line">
                                <span>{activeProfileTarget.name}</span>
                                <span className="badge btag">{activeProfileTarget.type}</span>
	                                <span className="badge btag">{activeProfileTarget.region}</span>
	                                <span className="badge btag">{TIER_LABEL[activeProfileTarget.tier]}권</span>
	                                {activeProfileTargetGoal && <span className="badge btag goal-badge">목표대학</span>}
	                                {!activeProfileTargetSelected && <span className="badge btag">검색 미리보기</span>}
	                              </div>
	                              <div className="profile-target-school-sub">국수영 평균과 전년도 입결 기준으로 전형 적합도를 확인합니다.</div>
	                            </div>
	                            {activeProfileTargetSelected ? (
	                              <div className="profile-target-actions">
	                                <button
	                                  type="button"
	                                  className={`profile-goal-toggle detail${activeProfileTargetGoal ? " active" : ""}`}
	                                  onClick={() => setProfileTargetGoal(activeProfileTarget.id, !activeProfileTargetGoal)}
	                                  disabled={!activeProfileTargetGoal && profileGoalLimitReached}
	                                >
	                                  {activeProfileTargetGoal ? "목표 해제" : profileGoalLimitReached ? "목표 2개" : "목표로 표시"}
	                                </button>
	                                <button className="assignment-delete-btn" type="button" onClick={() => removeProfileTarget(activeProfileTarget.id)}>삭제</button>
	                              </div>
	                            ) : (
	                              <button className="small-primary profile-target-add-btn" type="button" onClick={() => addProfileTarget(activeProfileTarget)} disabled={profileInterestLimitReached}>
	                                {profileInterestLimitReached ? "10개 선택됨" : "관심대학 추가"}
	                              </button>
	                            )}
                          </div>

                          <div className="profile-target-control-row">
                            <div className="field">
                              <label htmlFor={`profile-target-dept-${activeProfileTarget.id}`}>관심 학과</label>
                              <select id={`profile-target-dept-${activeProfileTarget.id}`} className="auth-input" value={activeProfileTargetDept} onChange={e => setActiveProfileTargetDept(e.target.value)}>
                                <option value="">학과 선택</option>
                                {activeProfileTargetDepartments.map(dept => <option key={dept} value={dept}>{dept}</option>)}
                              </select>
                            </div>
                            {activeProfileTargetMatch && (
                              <div className="minfo profile-target-match" style={{ background:activeProfileTargetMatch.bg, borderColor:activeProfileTargetMatch.border }}>
                                <div><div style={{ fontSize:11, color:"#6B7684", marginBottom:2 }}>국·수·영 평균</div><div style={{ fontSize:22, fontWeight:700, color:"#202632", letterSpacing:0 }}>{profileStudent.avgValue ? (+profileStudent.avgValue).toFixed(1) : "-"}등급</div></div>
                                <div style={{ fontSize:18, color:"#C9D3DF" }}>→</div>
                                <div>
                                  <div style={{ fontSize:11, color:"#6B7684", marginBottom:2 }}>합격 가능성</div>
                                  <div style={{ fontSize:16, fontWeight:600, color:activeProfileTargetMatch.color }}>{activeProfileTargetMatch.label}</div>
                                  <div className="mbar" style={{ width:140 }}><div style={{ height:"100%", width:`${activeProfileTargetMatch.pct}%`, background:activeProfileTargetMatch.color, borderRadius:3 }} /></div>
                                </div>
                              </div>
                            )}
                          </div>

                          {activeProfileAdmissionStats && (
                            <div className="profile-target-analysis-card">
                              <div className="admission-head">
                                <div>
                                  <div className="secl">{activeProfileAdmissionStats.year} 수시 합격 통계</div>
                                  <div className="admission-title">{activeProfileAdmissionStats.dept}</div>
                                  <div className="admission-copy">{activeProfileAdmissionStats.info}</div>
                                </div>
                                <div className="admission-range">
                                  <span>전년도 합격권</span>
                                  <strong>{activeProfileAdmissionStats.gradeRange}</strong>
                                </div>
                              </div>
                              <div className="admission-grid">
                                <div className="admission-stat">
                                  <div className="admission-stat-label">학생부 일반 평균</div>
                                  <div className="admission-stat-value admission-value-blue">{activeProfileAdmissionStats.generalStats?.avgGrade ? `${activeProfileAdmissionStats.generalStats.avgGrade.toFixed(1)}등급` : "-"}</div>
                                </div>
                                <div className="admission-stat">
                                  <div className="admission-stat-label">학생부종합 평균</div>
                                  <div className="admission-stat-value admission-value-orange">{activeProfileAdmissionStats.comprehensiveStats?.avgGrade ? `${activeProfileAdmissionStats.comprehensiveStats.avgGrade.toFixed(1)}등급` : "-"}</div>
                                </div>
                                <div className="admission-stat">
                                  <div className="admission-stat-label">평균 경쟁률</div>
                                  <div className="admission-stat-value">{activeProfileAdmissionStats.competition}:1</div>
                                </div>
                                <div className="admission-stat">
                                  <div className="admission-stat-label">강한 전형</div>
                                  <div className="admission-stat-value" style={{ fontSize:14 }}>{activeProfileAdmissionStats.bestTrack}</div>
                                </div>
                              </div>
                              <div className="admission-table-wrap">
                                <table className="admission-table">
                                  ...
                                  <tbody>
                                    {activeProfileAdmissionStats.rows.map(row => (
                                      <tr key={row.name}>
                                        <td>
                          ...
                          {activeProfileDepartmentStats.length > 0 && (
                            <div className="profile-target-analysis-card">
                              <div className="secl">학과별 수시 합격 등급표</div>
                              <div className="mini-body" style={{ marginBottom:10 }}>선택한 대학에 연결된 학과만 표시합니다.</div>
                              <div className="dept-stat-list profile-dept-stat-list">
                                {activeProfileDepartmentStats.map(item => (
                                  <button
                                    key={item.dept}
                                    type="button"
                                    className={`dept-stat-row${activeProfileTargetDept === item.dept ? " active" : ""}`}
                                    onClick={() => setActiveProfileTargetDept(item.dept)}
                                  >
                          ...
                          <div className="tabs">
                            {["수시","정시"].map(tab => <button key={tab} type="button" className={`tab${profileTargetTrackTab === tab ? " active" : ""}`} onClick={() => setProfileTargetTrackTab(tab)}>{tab}</button>)}
                          </div>
                          {tracksForTab(activeProfileTarget.tracks, profileTargetTrackTab).map((track, index) => (
                            <div key={`${track.name}-${index}`} className="tcard profile-target-track-card">
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                                <div>
                                  <div style={{ fontSize:14.5, fontWeight:600, color:"#202632", marginBottom:3 }}>{track.name}</div>
                                  <div style={{ fontSize:12.5, color:"#6B7684" }}>{track.note}</div>
                                </div>
                                <span className={`badge b${track.type}`}>{track.type}</span>
                              </div>
                              <div className="sgrid">
                                <div className="sbox"><div className="slabel">권장 등급</div><div className="sval">{track.grade ? `${track.grade}등급` : "수능 반영"}</div></div>
                                <div className="sbox"><div className="slabel">경쟁률</div><div className="sval">{track.comp}:1</div></div>
                                <div className="sbox"><div className="slabel">모집인원</div><div className="sval">{track.quota}명</div></div>
                              </div>
                            </div>
                          ))}
                          {!tracksForTab(activeProfileTarget.tracks, profileTargetTrackTab).length && <div className="empty profile-target-empty">해당 전형 데이터가 없습니다</div>}
                        </>
                      ) : (
	                        <div className="empty profile-target-empty">대학명을 검색하여 관심대학을 추가하세요</div>
                      )}
                    </div>
                  </div>
                </section>
                ...
                {profileRoadmapState && (
                    <section className="profile-section counselor-roadmap">
                      <div className="request-top">
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", justifyContent:"flex-end" }}>
                          <button className={`toggle-roadmap-btn${roadmapOpen ? " open" : ""}`} type="button" onClick={() => setRoadmapOpen(open => !open)}>
                            {roadmapOpen ? "줄이기" : "펼치기"}
                          </button>
                      ...
                      {roadmapOpen ? (
                        ...
                        <button className="roadmap-compact-summary" type="button" onClick={() => setRoadmapOpen(true)}>
                          <span className="roadmap-compact-progress">{profileRoadmapState.progress}%</span>
                          <span>
                            <strong>{profileRoadmapState.nextItem ? profileRoadmapState.nextItem.title : "모든 미션 완료"}</strong>
                            <small>{profileRoadmapState.nextItem ? `${profileRoadmapState.nextItem.month} · ${profileRoadmapState.nextItem.body}` : "상담 전 준비가 정리되었습니다."}</small>
                          </span>
                          <em>{profileRoadmapState.urgent.length ? `긴급 ${profileRoadmapState.urgent.length}` : "펼치기"}</em>
                        </button>
                      )}
                    </section>
                  )}
	                <div className="profile-grid">
	                  <div>
	                    <section className="profile-section grade-entry-section">
	                      <div className="grade-section-head">
	                        <div>
	                          <div className="secl">성적표</div>
	                          <div className="mini-body">학년별로 접고 펼치며 중간·기말 원점수와 등급을 입력합니다.</div>
	                        </div>
	                        <span className={`score-pill ${profileStudent.avgValue ? (+profileStudent.avgValue <= 2 ? "good" : +profileStudent.avgValue <= 4 ? "mid" : "warn") : ""}`}>
	                          {profileStudent.avgValue ? `국수영 평균 ${(+profileStudent.avgValue).toFixed(1)}등급` : "성적 미입력"}
	                        </span>
	                      </div>
	                      ...
	                      <div className="grade-year-stack">
	                        {GRADE_YEAR_GROUPS.map(({ year, sems }) => {
	                          const hasYearData = sems.some(semester => SUBJECTS.some(subject => (
	                            gradeEntryValue(profileStudent.profile.grades, subject, semester) !== ""
	                            || EXAM_SCORE_TYPES.some(type => examScoreValue(profileStudent.profile.grades, subject, semester, type.key) !== "")
	                          )));
	                          const isOpen = profileExpandedGradeYears.includes(year);
	                          const summaries = sems
	                            .map(semester => {
	                              const average = semAvgForGrades(profileStudent.profile.grades || {}, semester);
	                              return average ? `${semester} ${average}등급` : null;
	                            })
	                            .filter(Boolean);
	                          const toggleYear = () => setProfileExpandedGradeYears(prev =>
	                            prev.includes(year) ? prev.filter(item => item !== year) : [...prev, year]
	                          );
	                          return (
	                            <div key={year} className={`grade-year-block${isOpen ? " open" : ""}`}>
	                              <button className="grade-year-toggle" type="button" onClick={toggleYear}>
	                                <span className="grade-year-label">{year}</span>
	                                {summaries.length ? <span className="grade-year-badge">{summaries.join(" · ")}</span> : <span className="grade-year-empty">{hasYearData ? "원점수 입력됨" : "미입력"}</span>}
	                                <span className="grade-year-chevron">{isOpen ? "접기" : "펼치기"}</span>
	                              </button>
	                              {isOpen && (
	                                <div className="grade-year-body">
	                                  <div className="grade-sem-grid" style={{ gridTemplateColumns: sems.length === 1 ? "1fr" : "1fr 1fr" }}>
	                                    {sems.map(semester => (
	                                      <div key={semester} className="grade-sem-col">
	                                        <div className="grade-sem-header">
	                                          <span>{semester}</span>
	                                          <em>{semAvgForGrades(profileStudent.profile.grades || {}, semester) ? `국수영 ${semAvgForGrades(profileStudent.profile.grades || {}, semester)}등급` : "국수영 미입력"}</em>
	                                        </div>
	                                        <table className="gtable grade-compact-table">
	                                          <thead>
	                                            <tr>
	                                              <th style={{ textAlign:"left", width:70 }}>과목</th>
	                                          ...
	                                          <tbody>
	                                            {SUBJECTS.map(subject => (
	                                              <tr key={subject}>
	                                                <td>
	                                                  <div className="sn">
	                                                    {LINE_COLORS[subject] && <span className="dot" style={{ background:LINE_COLORS[subject], width:6, height:6 }} />}
	                                                    <span style={{ fontWeight:CORE.includes(subject) ? 800 : 500, fontSize:13 }}>{subject}</span>
	                                                  </div>
	                                                </td>
	                                                <td>
	                                                  <input
	                                                    ...
	                                                    max="100"
	                                                    step="1"
	                                                    value={examScoreValue(profileStudent.profile.grades, subject, semester, "midterm")}
	                                                    placeholder="-"
	                                                    onChange={e => setProfileExamScore(subject, semester, "midterm", e.target.value)}
	                                                    aria-label={`${subject} ${semester} 중간고사`}
	                                                  />
	                                                </td>
	                                                <td>
	                                                  <input
	                                                    ...
	                                                    max="100"
	                                                    step="1"
	                                                    value={examScoreValue(profileStudent.profile.grades, subject, semester, "final")}
	                                                    placeholder="-"
	                                                    onChange={e => setProfileExamScore(subject, semester, "final", e.target.value)}
	                                                    aria-label={`${subject} ${semester} 기말고사`}
	                                                  />
	                                                </td>
	                                                <td>
	                                                  <input
	                                                    className="grade-required"
	                                                    type="number"
	                                                    min="1"
	                                                    max="9"
	                                                    step="0.1"
	                                                    value={gradeEntryValue(profileStudent.profile.grades, subject, semester)}
	                                                    placeholder="등급"
	                                                    onChange={e => setProfileGrade(subject, semester, e.target.value)}
	                                                    aria-label={`${subject} ${semester} 등급`}
	                                                  />
	                                                </td>
	                                              </tr>
	                                            ))}
	                      ...
	                      <div className="grade-average-panel">
	                        <div className="grade-average-head">
	                          <div>
	                            <div className="secl">학기별 국수영 평균</div>
	                            <div className="mini-body">성적표 입력값을 기준으로 국어·수학·영어만 평균냅니다.</div>
	                          </div>
	                          {profileHasSemesterAverages && <span className="status-pill sent">자동 계산</span>}
	                        </div>
	                        <div className="grade-average-grid">
	                          {profileSemesterAverages.map(item => (
	                            <div key={item.semester} className={`grade-average-card${item.avg ? "" : " empty"}`}>
	                              <span>{item.semester}</span>
	                              <strong>{item.avg || "-"}</strong>
	                              <small>{item.avg ? "등급" : "미입력"}</small>
	                            </div>
	                          ))}
	                        </div>
	                      </div>
	                    ...
	                    {profileHasGrades && (
	                      <section className="profile-section grade-chart-card">
	                        <div className="grade-chart-head">
	                          <div>
	                            <div className="secl">국수영 성적 추이</div>
	                            <div className="mini-body">그래프는 주요 과목 3개만 표시합니다. 1등급에 가까울수록 우수합니다.</div>
	                          </div>
	                        </div>
	                        <ResponsiveContainer width="100%" height={260}>
	                          <LineChart data={profileChartData} margin={{ top:16, right:24, bottom:0, left:-8 }}>
	                            <CartesianGrid strokeDasharray="3 3" stroke="#EEF2F7" vertical={false} />
	                            <XAxis dataKey="semester" tick={{ fontSize:12, fill:"#9AA6B2" }} axisLine={false} tickLine={false} />
	                            <YAxis
	                              domain={profileGradeChartDomain}
	                              ticks={profileGradeChartTicks}
	                              reversed={true}
	                              tickFormatter={v => `${Number(v).toFixed(Number.isInteger(v) ? 0 : 1)}등`}
	                            ...
	                            />
	                            <Tooltip
	                              formatter={(v,n)=>[`${(+v).toFixed(1)}등급`,n]}
	                              contentStyle={{ background:"#fff", border:"1px solid #E5EAF1", borderRadius:12, fontSize:13, boxShadow:"0 10px 28px rgba(11,19,36,0.10)" }}
	                              labelStyle={{ color:"#6B7684", fontWeight:800, marginBottom:4 }}
	                            />
	                            <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize:12, color:"#6B7684", paddingTop:12 }} />
	                            {CORE.map(subject => <Line key={subject} type="monotone" dataKey={subject} stroke={LINE_COLORS[subject]} strokeWidth={3} dot={{ r:5, fill:LINE_COLORS[subject], strokeWidth:2, stroke:"#fff" }} activeDot={{ r:7, strokeWidth:2, stroke:"#fff" }} connectNulls />)}
	                          </LineChart>
	                        </ResponsiveContainer>
	                      </section>
                    )}
                ...
                <section className="profile-section counseling-journal-section">
                  <div className="request-top">
                    <div>
                      <div className="secl">상담 기록</div>
              ...
              <>
                ...
                <div className="student-admin-actions" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap", marginBottom:16 }}>
                  <div />
                  <button className="small-primary" type="button" onClick={() => setShowNewStudentForm(prev => !prev)}>
                    {showNewStudentForm ? "추가 취소" : "새 학생 추가"}
                  </button>
                </div>
                {showNewStudentForm && (
                  <section className="profile-section counselor-edit-panel" style={{ marginBottom:20 }}>
                    <div className="request-top">
                      <div>
                        <div className="secl">학생 추가</div>
                        <div className="mini-body">담당 학생을 추가하고 필요한 경우 반을 지정할 수 있습니다.</div>
                      </div>
                      <span className="status-pill sent">새 학생</span>
                    </div>
                    <div className="form-grid">
                      <div className="field">
                        <label htmlFor="new-student-name">이름</label>
                        <input id="new-student-name" className="auth-input" value={newStudentForm.name} onChange={e => setNewStudentForm(prev => ({ ...prev, name: e.target.value }))} />
                      </div>
                      <div className="field">
                        <label htmlFor="new-student-school">고등학교</label>
                        <input id="new-student-school" className="auth-input" value={newStudentForm.highSchool} onChange={e => setNewStudentForm(prev => ({ ...prev, highSchool: e.target.value }))} />
                      </div>
                      <div className="field">
                        <label htmlFor="new-student-major">희망 학과</label>
                        <input id="new-student-major" className="auth-input" value={newStudentForm.preferredMajor} onChange={e => setNewStudentForm(prev => ({ ...prev, preferredMajor: e.target.value }))} />
                      </div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                        <div className="field">
                          <label htmlFor="new-student-grade">학년</label>
                          <select id="new-student-grade" className="auth-input" value={newStudentForm.gradeLevel} onChange={e => setNewStudentForm(prev => ({ ...prev, gradeLevel: e.target.value }))}>
                            <option value="1학년">1학년</option>
                            <option value="2학년">2학년</option>
                            <option value="3학년">3학년</option>
                          </select>
                        ...
                        <div className="field">
                          <label htmlFor="new-student-class">반</label>
                          <select id="new-student-class" className="auth-input" value={newStudentForm.classId} onChange={e => setNewStudentForm(prev => ({ ...prev, classId: e.target.value }))}>
                            <option value="">반 선택 안함</option>
                            {myCounselorClasses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                          </select>
                        </div>
                      </div>
                    </div>
                    <div className="assignment-form-actions" style={{ justifyContent:"flex-end" }}>
                      <button className="small-primary" type="button" onClick={createNewStudent}>학생 추가</button>
                    </div>
                  </section>
                )}
                            ...
                            <span className={`score-pill ${avgValue ? (+avgValue <= 2 ? "good" : +avgValue <= 4 ? "mid" : "warn") : ""}`}>{avgValue ? `${(+avgValue).toFixed(1)}등급` : "미입력"}</span>
	                          ...
	                          <div>
	                            ...
	                            <div className="target-stack">
	                              {goalTargets.map(t => <span key={t.id} className="badge btag">{t.short}{t.dept ? ` · ${t.dept}` : ""}</span>)}
	                              {(profile.targets || []).length > goalTargets.length && <span className="badge btag">관심 +{(profile.targets || []).length - goalTargets.length}</span>}
	                              {!goalTargets.length && <span style={{ fontSize:12, color:"#9AA6B2" }}>없음</span>}
	                            </div>
	                          </div>
                          <div>
        ...
        <main className="main">
          ...
          {section === "홈" && <>
            <div className="home-hero">
              ...
              <div className="home-sub">목표 대학, 성적표, 전략센터, 생활기록부, 자소서, 로드맵, 과제 점검을 앱 흐름처럼 한 화면에서 이어서 관리합니다.</div>
            </div>
          ...
          {/* 성적표 */}
	          {section === "성적표" && <>
	            ...
	            <div className="psub">등급은 필수 기준입니다. 중간/기말 원점수는 기억나는 경우만 입력합니다.</div>
	            <div className="grade-overview-pill">
	              <span>국수영 전체 평균</span>
	              <strong>{avg ? `${(+avg).toFixed(1)}등급` : "미입력"}</strong>
	            </div>
	            ...
	            <section className="card grade-entry-section">
	              <div className="grade-section-head">
	                <div>
	                  <div className="secl">학년별 성적 입력</div>
	                  <div className="mini-body">현재 학년 중심으로 열어두고, 다음 학년은 필요할 때 펼쳐 입력합니다.</div>
	                </div>
	              </div>
	              <div className="grade-year-stack">
	            {GRADE_YEAR_GROUPS.map(({ year, sems }) => {
	              const hasYearData = sems.some(m => SUBJECTS.some(s => (
	                getG(s,m) !== "" || EXAM_SCORE_TYPES.some(type => getExamScore(s, m, type.key) !== "")
	              )));
	              const isOpen = expandedGradeYears.includes(year);
	              const toggleYear = () => setExpandedGradeYears(prev =>
	              ...
	              return (
	                <div key={year} className={`grade-year-block${isOpen ? " open" : ""}`}>
	                  <button className="grade-year-toggle" type="button" onClick={toggleYear}>
	                    <span className="grade-year-label">{year}</span>
                    {hasYearData && (
                      <span className="grade-year-badge">
                        {sems.map(m => { const a = semAvg(m); return a ? `${m} ${a}등급` : null; }).filter(Boolean).join("  ·  ")}
                      </span>
	                    )}
	                    {!hasYearData && <span className="grade-year-empty">미입력</span>}
	                    <span className="grade-year-chevron">{isOpen ? "접기" : "펼치기"}</span>
	                  </button>
                  {isOpen && (
                    <div className="grade-year-body">
                      <div className="grade-sem-grid" style={{ gridTemplateColumns: sems.length === 1 ? "1fr" : "1fr 1fr" }}>
                        {sems.map(sem => (
                          <div key={sem} className="grade-sem-col">
	                            <div className="grade-sem-header">
	                              <span>{sem}</span>
	                              <em>{semAvg(sem) ? `국수영 ${semAvg(sem)}등급` : "국수영 미입력"}</em>
	                            </div>
                            <table className="gtable grade-compact-table">
                              <thead>
                                <tr>
                                  <th style={{ textAlign:"left", width:60 }}>과목</th>
                                  <th className="grade-subhead">중간</th>
                                  <th className="grade-subhead">기말</th>
                                  <th className="grade-subhead grade-required-head">등급</th>
                                </tr>
                              ...
                              <tbody>
                                {SUBJECTS.map(s => (
                                  <tr key={s}>
                                    <td>
                                      <div className="sn">
                                        {LINE_COLORS[s] && <span className="dot" style={{ background:LINE_COLORS[s], width:6, height:6 }} />}
                                        <span style={{ fontWeight:CORE.includes(s)?700:400, fontSize:13 }}>{s}</span>
                                      </div>
                                    </td>
                                    <td><input type="number" min="0" max="100" step="1" value={getExamScore(s,sem,"midterm")} placeholder="—" onChange={e => setExamScore(s,sem,"midterm",e.target.value)} aria-label={`${s} ${sem} 중간고사`} /></td>
                                    <td><input type="number" min="0" max="100" step="1" value={getExamScore(s,sem,"final")} placeholder="—" onChange={e => setExamScore(s,sem,"final",e.target.value)} aria-label={`${s} ${sem} 기말고사`} /></td>
	                                    <td><input className="grade-required" type="number" min="1" max="9" step="0.1" value={getG(s,sem)} placeholder="등급" onChange={e => setG(s,sem,e.target.value)} aria-label={`${s} ${sem} 등급`} /></td>
	                                  </tr>
                                ))}
                              </tbody>
                            </table>
	            ...
	            <section className="card grade-average-panel">
	              <div className="grade-average-head">
	                <div>
	                  <div className="secl">학기별 국수영 평균</div>
	                  <div className="mini-body">성적표 바로 아래에서 학기별 흐름을 확인합니다.</div>
	                </div>
	                {hasSemesterAverages && <span className="status-pill sent">자동 계산</span>}
	              </div>
	              <div className="grade-average-grid">
	            ...
	            {/* 성적 추이 그래프 */}
	            {hasGrades ? (
	              <div className="card grade-chart-card" style={{ marginTop:12 }}>
	                <div className="grade-chart-head">
	                  <div>
	                    <div className="secl">국수영 성적 추이</div>
	                    <div className="mini-body">주요 과목 3개만 표시해 변화가 더 잘 보이도록 했습니다.</div>
	                  </div>
	                </div>
                    ...
                    <YAxis
	                      ...
	                      reversed={true}
	                      ticks={gradeChartTicks}
	                      tickFormatter={v => `${Number(v).toFixed(Number.isInteger(v) ? 0 : 1)}등`}
                      tick={{ fontSize:11, fill:"#C9D3DF" }}
                      axisLine={false}
                      tickLine={false}
                      width={36}
                    />
                    <Tooltip
                      formatter={(v, n) => [`${(+v).toFixed(1)}등급`, n]}
                      contentStyle={{ background:"#fff", border:"1px solid #E5EAF1", borderRadius:12, fontSize:13, boxShadow:"0 4px 16px rgba(11,19,36,0.08)" }}
                      labelStyle={{ color:"#6B7684", fontWeight:600, marginBottom:4 }}
                    />
                    <Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize:12, color:"#6B7684", paddingTop:12 }} />
	                    {CORE.map(s => (
	                      <Line
	                        key={s}
	                        type="monotone"
	                        ...
	                        strokeWidth={3}
	                        dot={{ r:5, fill:LINE_COLORS[s], strokeWidth:2, stroke:"#fff" }}
