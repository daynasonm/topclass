...
export const createBackendStudent = async (student) => {
  if (!supabase || !student?.id) return null;
  const row = {
    id: student.id,
    name: student.name || "학생",
    email: student.email || `student-${student.id}@topclass.local`,
    role: "student",
    is_active: student.isActive !== false,
    grade_level: student.gradeLevel || null,
    class_name: student.className || null,
    high_school: student.highSchool || null,
    preferred_major: student.preferredMajor || null,
  };
...
export const createBackendClassMembership = async (studentId, classId) => {
  if (!supabase || !studentId || !classId) return;
  const { error } = await supabase
    .from("class_memberships")
    .upsert({ student_id: studentId, class_id: classId, joined_at: new Date().toISOString() }, { onConflict: "student_id" });
  if (error) throw error;
};
